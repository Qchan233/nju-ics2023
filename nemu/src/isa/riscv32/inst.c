...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4, s->dnpc = SET0(s->pc+imm), check_call(s->pc, s->dnpc));
  
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4, s->dnpc = SET0(src1+imm), /*check_call(s->pc, s->dnpc),*/ check_return(s->pc,INSTPAT_INST(s)));
