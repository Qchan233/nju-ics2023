...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4, s->dnpc = SET0(s->pc+imm), check_call(s->pc, s->dnpc, rd));
  
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4, s->dnpc = SET0(src1+imm), check_call(s->pc, s->dnpc, rd), check_return(s->pc,INSTPAT_INST(s)));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh  , R, R(rd) = (word_t) (((long long) (int32_t)  src1 * (long long) (int32_t) src2) >> 32)); //Use sign extension
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R, R(rd) = (word_t) (((long long) (int32_t)  src1 * (long long) (uint32_t) src2) >> 32)); //Use sign extension
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu , R, R(rd) = (word_t) (((long long) (uint32_t)  src1 * (long long) (uint32_t) src2) >> 32)); //Use sign extension
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(env_call(cpu.level), s->pc)); 
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = cpu.mepc + 4, cpu.level = U); 
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = get_csr_value(imm), set_csr_value(imm, src1));
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = get_csr_value(imm), set_csr_value(imm, get_csr_value(imm) | src1));
